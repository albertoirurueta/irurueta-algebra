...
class FrobeniusNormComputerTest {
    ...
    void testGetNormMatrix() throws WrongSizeException {
        ...
        var sum = 0.0;
        double norm;
        var initValue = randomizer.nextDouble(MIN_COLUMNS, MAX_COLUMNS);
        double value;
    ...
    void testNormWithJacobian() throws AlgebraException {
        ...
        assertEquals(Matrix.newFromArray(v).multiplyByScalarAndReturnNew(1.0 / norm).transposeAndReturnNew(), jacobian);

        // Force WrongSizeException
        final var m1 = new Matrix(2, length);
        assertThrows(WrongSizeException.class, () -> FrobeniusNormComputer.norm(v, m1));
